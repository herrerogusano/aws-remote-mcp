...
| Excessive AWS discovery | Fixed Lambda/API inventory; separate CloudFormation inventory restricted to the four exact project stacks and stack-level `ListStackResources`; opt-in Resource Explorer on one exact view; positive query grammar and no caller-controlled pagination | CloudFormation results cover only direct resources in the four project stacks and can be partial on timeout; Resource Explorer remains eventually consistent and non-authoritative |
...
| Supply-chain regression | Locked dependencies, pinned CI actions, formatting, static typing, 222 offline tests and SAM build validation | Upstream package or action compromise cannot be eliminated entirely |
