...
| Excessive AWS discovery | Fixed two-service inventory plus opt-in Resource Explorer on one exact view; positive query grammar, one page, at most 50 results and one SDK attempt | Results are intentionally incomplete and depend on eventually consistent indexes |
